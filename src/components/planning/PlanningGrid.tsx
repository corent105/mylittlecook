...
function DroppableSlot({ slotId, children, isPastDay = false }: {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`w-full h-full ${isOver && !isPastDay ? 'bg-orange-50 ring-2 ring-orange-300 rounded-md' : ''}`}
    >
      {children}
    </div>
  );
...
export default function PlanningGrid({
  ...
  const renderMealCard = (meal: any, isMobile = false) => (
    <DraggableMealCard key={meal.id} meal={meal}>
      <div
        className="relative group bg-white rounded border border-orange-100 p-2 shadow-sm hover:shadow-md transition-all cursor-pointer"
        onClick={(e) => {
          // Éviter le conflit avec le drag
          e.stopPropagation();
